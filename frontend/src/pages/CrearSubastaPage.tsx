...
export default function CrearSubastaPage() {
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!file) return;
    if (file.type !== 'image/jpeg' && file.type !== 'image/png') { setUploadError('Solo se permiten imágenes JPG y PNG'); return; }
    if (file.size > MAX_SIZE_MB * 1024 * 1024) { setUploadError(`La imagen no debe superar los ${MAX_SIZE_MB} MB`); return; }
    setUploadError('');
    setUploading(true);
    try {
  ...
  return (
    <div className="max-w-xl mx-auto bg-surface border border-border p-6 rounded-2xl animate-fade-in">
      ...
      {error && (
        <div className="mb-4 rounded-xl bg-rose-500/10 border border-rose-500/20 px-4 py-3 text-xs text-rose-400">{error}</div>
      )}
      <div className="border-b border-border pb-3 mb-5">
        ...
        <p className="text-[11px] text-text-muted">Crea el producto y regístralo como BORRADOR</p>
      </div>
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            ...
            <div className="relative">
              ...
              <input type="number" required value={newBasePrice} onChange={(e) => setNewBasePrice(e.target.value)} className="w-full rounded-xl border border-border bg-input py-3 pl-8 pr-3 text-text-primary focus:border-amber-500 focus:outline-none font-mono" placeholder="250" min="1" />
            </div>
          </div>
          <div>
            ...
            <div className="relative">
              <div className="pointer-events-none absolute inset-y-0 left-0 flex items-center pl-3"><span className="text-text-muted">$</span></div>
              <input type="number" required value={newMinIncrement} onChange={(e) => setNewMinIncrement(e.target.value)} className="w-full rounded-xl border border-border bg-input py-3 pl-8 pr-3 text-text-primary focus:border-amber-500 focus:outline-none font-mono" placeholder="10" min="1" />
            </div>
          </div>
        </div>
        <div>
          ...
          <div className="flex gap-2">
            <input type="url" value={newImage} onChange={(e) => setNewImage(e.target.value)} className="flex-1 rounded-xl border border-border bg-input p-3 text-text-primary placeholder-slate-600 focus:border-amber-500 focus:outline-none" placeholder="https://... (URL) o subí un archivo" />
            <button type="button" onClick={() => fileInputRef.current?.click()} disabled={uploading} className="flex items-center gap-1.5 rounded-xl border border-border bg-input px-4 py-3 text-xs font-medium text-text-secondary hover:text-amber-400 hover:border-amber-500/30 transition disabled:opacity-50">
