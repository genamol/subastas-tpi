          ...
          <div className="bg-surface border border-border rounded-2xl overflow-hidden relative">
            ...
            {showCountdown && (
              <div className={`absolute top-4 right-4 flex items-center space-x-2 rounded-full border px-3 py-1.5 text-sm font-bold text-white ${
                ...
                <span className={isUrgent ? 'text-rose-300' : isEnded ? 'text-text-secondary' : 'text-emerald-300'}>
                  {timeLeft}
                </span>
              </div>
            )}
          ...
          <div className="bg-surface border border-border p-5 rounded-2xl">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-6 border-t border-border/60 pt-4 text-xs">
              <div>
                ...
                <strong className="text-text-primary mt-0.5 block">{auction.category}</strong>
              </div>
              <div>
                <span className="block text-[9px] text-text-muted uppercase">Vendedor</span>
                <strong className="text-text-primary mt-0.5 block">{auction.seller.name}</strong>
              </div>
              <div>
              ...
              <div>
                ...
                <strong className="text-text-secondary mt-0.5 block font-mono">{formatDate(auction.createdAt)}</strong>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="space-y-4">
          <div className="bg-surface border border-border p-5 rounded-2xl">
            ...
            <div className="space-y-3 mb-5 text-xs text-text-secondary">
              <div className="flex items-center justify-between">
                ...
                <strong className="text-text-primary">{auction.bidsCount}</strong>
              </div>
              <div className="flex items-center justify-between">
          ...
          <div className="bg-surface border border-border p-5 rounded-2xl">
            ...
            {!showDisputeForm ? (
              ...
              <form onSubmit={(e) => { e.preventDefault(); onSubmitDispute(); }} className="space-y-3 mt-3 border-t border-border/60 pt-3 text-xs">
                <div>
