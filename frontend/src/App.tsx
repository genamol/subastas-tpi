    ...
    <div className="min-h-screen bg-[#0A0A0C] text-slate-100 font-sans antialiased selection:bg-amber-500 selection:text-black">
      ...
      {successMessage && (
        <div className="fixed top-4 left-1/2 -translate-x-1/2 z-50 max-w-md w-full px-4 animate-fade-in">
          <div className="flex items-center space-x-3 rounded-2xl border border-emerald-500/20 bg-[#0F0F13]/90 backdrop-blur-md p-4 text-xs font-medium text-emerald-400 shadow-2xl shadow-emerald-500/10">
      ...
      <header className="sticky top-0 z-40 w-full border-b border-slate-800 bg-[#0A0A0C]/85 backdrop-blur-md">
        <div className="mx-auto flex h-16 max-w-7xl items-center justify-between px-4 sm:px-6 lg:px-8">
          ...
          <div className="flex items-center space-x-3">
            ...
            <div className="flex items-center space-x-2.5 rounded-xl border border-slate-800 bg-[#0F0F13] py-1 pl-3 pr-2">
            ...
            <div className="relative">
              <button
                ...
                className={`relative flex h-9 w-9 items-center justify-center rounded-xl border transition-colors cursor-pointer ${
                  showNotifications 
                    ...
                    : 'border-slate-800 bg-[#0F0F13] text-slate-400 hover:text-slate-200'
                }`}
              >
              ...
              {showNotifications && (
                <div className="absolute right-0 mt-2 w-72 rounded-2xl border border-slate-800 bg-[#0F0F13] p-2 shadow-2xl z-50 animate-fade-in max-h-80 overflow-y-auto">
                  <div className="flex items-center justify-between px-3 py-2 border-b border-slate-800/60 mb-1">
                    <span className="text-[10px] font-bold uppercase tracking-wider text-slate-500">Avisos del Sistema</span>
                    <button 
                  ...
                  {notifications.length === 0 ? (
                    <span className="block py-6 text-center text-xs text-slate-600">No tienes notificaciones</span>
                  ) : (
                    notifications.map(n => (
                      <div 
                        key={n.id} 
                        className={`p-2.5 rounded-xl text-xs transition-colors mb-1 ${
                          n.read ? 'opacity-55' : 'bg-slate-900/40 border-l-2 border-amber-500'
                        }`}
                      >
                        <div className="flex items-center justify-between mb-0.5">
                          <span className="font-bold text-slate-200">{n.title}</span>
                          <span className="text-[9px] font-mono text-slate-600">{new Date(n.time).toLocaleTimeString()}</span>
                        </div>
          ...
          {currentTab === 'create' && !selectedAuction && (
            <div className="max-w-xl mx-auto bg-[#0F0F13] border border-slate-800 p-6 rounded-2xl animate-fade-in">
              ...
              <form onSubmit={handleCreateAuction} className="space-y-4 text-xs">
                ...
                <div>
                  <label className="block text-slate-400 font-bold mb-1.5 uppercase tracking-wider">URL de la Imagen (Opcional):</label>
                  <input
                    type="url"
                    value={newImage}
          ...
          {selectedAuction && (
            <div className="animate-fade-in space-y-6">
              <div className="flex items-center space-x-2 text-xs">
                ...
                <span className="text-slate-500 font-medium truncate max-w-[200px]">{selectedAuction.title}</span>
              </div>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div className="lg:col-span-2 space-y-4">
                  <div className="bg-[#0F0F13] border border-slate-800 rounded-2xl overflow-hidden relative">
                    ...
                    <div className="absolute top-4 left-4 flex items-center space-x-1.5 rounded-full bg-slate-950/85 backdrop-blur-md px-3 py-1 text-xs font-semibold text-white">
                  ...
                  <div className="bg-[#0F0F13] border border-slate-800 p-5 rounded-2xl">
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-6 border-t border-slate-800/60 pt-4 text-xs">
                      <div>
                        ...
                        <strong className="text-slate-200 mt-0.5 block">{selectedAuction.category}</strong>
                      </div>
                      <div>
                        <span className="block text-[9px] text-slate-500 uppercase">Vendedor</span>
                        <strong className="text-slate-200 mt-0.5 block">{selectedAuction.seller.name}</strong>
                      </div>
                      <div>
                      ...
                      <div>
                        ...
                        <strong className="text-slate-400 mt-0.5 block font-mono">{formatDate(selectedAuction.createdAt)}</strong>
                      </div>
                    </div>
                  </div>
                </div>
                ...
                <div className="space-y-4">
                  <div className="bg-[#0F0F13] border border-slate-800 p-5 rounded-2xl">
                    ...
                    <div className="space-y-3 mb-5 text-xs text-slate-400">
                      <div className="flex items-center justify-between">
                        ...
                        <strong className="text-slate-200">{selectedAuction.bidsCount}</strong>
                      </div>
                      <div className="flex items-center justify-between">
                        <span>Incremento Mínimo:</span>
                        <strong className="text-slate-200 font-mono">+${selectedAuction.minIncrement}</strong>
                      </div>
                      <div className="flex items-center justify-between">
                  ...
                  <div className="bg-[#0F0F13] border border-slate-800 p-5 rounded-2xl">
                    ...
                    {!showDisputeForm ? (
                      ...
                      <form onSubmit={handleOpenDispute} className="space-y-3 mt-3 border-t border-slate-800/60 pt-3 text-xs">
                        <div>
      ...
      <footer className="mt-20 border-t border-slate-900 bg-[#07070A] py-8 text-center text-xs text-slate-600 leading-loose">
        <div className="mx-auto max-w-7xl px-4 flex flex-col items-center justify-center space-y-2">
          <div className="flex items-center space-x-2 text-slate-500">
            <Gavel className="h-4 w-4" />
            <span className="font-display font-semibold tracking-wide text-slate-400 uppercase text-[10px]">Portal de Subastas Universitarias (TPI)</span>
          </div>
          ...
          <span className="block text-[10px] text-slate-700 font-mono mt-2">© 2026 • UTN FRVM • Todos los derechos reservados de prueba</span>
        </div>
      </footer>
